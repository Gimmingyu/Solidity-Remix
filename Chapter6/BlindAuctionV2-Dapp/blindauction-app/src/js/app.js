	...
	handlePhase: function (event) {
		web3.eth.getAccounts(function (error, accounts) {
			...
			App.contracts.vote
				.deployed()
				.then(function (instance) {
					return instance.advancePhase({ from: account });
				})
				.then(function (result) {
					console.log(result);
					if (result) {
						if (parseInt(result.receipt.status) == 1) {
							...
							App.contracts.vote
								.deployed()
								.then(function (latestInstance) {
									return latestInstance.currentPhase();
								})
								.then(function (result) {
									console.log(
										"This is also working, new phase updated"
									);
									App.currentPhase = result;
								});
							return;
		...
		web3.eth.getAccounts(function (error, accounts) {
			...
			App.contracts.vote
				...
				.then(function (instance) {
					...
					return bidInstance.bid(bidValue, {
						from: account,
						value: web3.toWei(msgValue, "ether"),
					});
				})
				.then(function (result, err) {
					if (result) {
						...
						if (parseInt(result.receipt.status) == 1)
							toastr.info("Your Bid is Placed!", "", {
								iconClass: "toast-info notification0",
							});
						else
							toastr["error"](
								"Error in Bidding. Bidding Reverted!"
							);
					} else {
		...
		web3.eth.getAccounts(function (error, accounts) {
			...
			App.contracts.vote
				...
				.then(function (instance) {
					...
					return bidInstance.reveal(
						parseInt(bidRevealValue),
						bidRevealSecret,
						{ from: account }
					);
				})
				.then(function (result, err) {
					if (result) {
						console.log(result.receipt.status);
						if (parseInt(result.receipt.status) == 1)
							toastr.info("Your Bid is Revealed!", "", {
								iconClass: "toast-info notification0",
							});
						else
							toastr["error"](
								"Error in Revealing. Bidding Reverted!"
							);
					} else {
	...
	handleWinner: function () {
		...
		web3.eth.getAccounts(function (error, accounts) {
			...
			App.contracts.vote
				...
				.then(function (instance) {
					bidInstance = instance;
					return bidInstance.auctionEnd({ from: account });
				})
				.then(function (res) {
					console.log(res);
					...
					toastr.info(
						"Highest bid is " +
							...
							winner,
						"",
						{ iconClass: "toast-info notification3" }
					);
				})
				.catch(function (err) {
	...
	handleWithdraw: function () {
		if (App.currentPhase == 3) {
			...
			App.contracts.vote
				...
				.then(function (instance) {
					console.log(
						"Trying to call withdraw with currentAccount: " +
							App.currentAccount
					);
