App = {
	...
	initContract: function () {
		$.getJSON("MPC.json", function (data) {
			App.contracts.Payment = new App.web3.eth.Contract(
				data.abi,
				data.networks[App.network_id].address,
				{}
			);
			App.web3.eth
				.getBalance(App.contracts.Payment._address)
				.then((res) => {
					jQuery("#channel_balance").text(
						App.web3.utils.fromWei(res),
						"ether"
					);
				});
		});
	...
	bindEvents: function () {
		...
		$(document).on("click", "#transfer", function () {
			App.handleTransfer(
				jQuery("#receiveramount").val(),
				jQuery("#signedMessage").val()
			);
		});
		App.populateAddress();
	},
		...
		new Web3(App.url).eth.getAccounts((err, accounts) => {
			if (!err) {
				...
				App.web3.eth.getBalance(accounts[0]).then((res) => {
					jQuery("#sender_balance").text(
						App.web3.utils.fromWei(res),
						"ether"
					);
				});
				App.web3.eth.getBalance(accounts[1]).then((res) => {
					jQuery("#receiver_balance").text(
						App.web3.utils.fromWei(res),
						"ether"
					);
				});
	...
	handleSignedMessage: function (receiver, amount) {
		...
		var message = App.constructPaymentMessage(
			App.contracts.Payment._address,
			weiamount
		);
	...
	signMessage: function (message, amount) {
		App.web3.eth.getAccounts((err, accounts) => {
			App.web3.eth.personal.sign(
				message,
				accounts[0],
				function (err, signedMessage) {
					if (!err) {
						var box =
	...
	handleTransfer: function (amount, signedMessage) {
		...
		if (App.web3.utils.isHexStrict(signedMessage)) {
			...
			App.contracts.Payment.methods
				.claimPayment(amount, signedMessage)
				.send(option)
				.on("receipt", (receipt) => {
					if (receipt.status) {
